...
const buildTextCanvas = ({
  ...
  Object.assign(probe.style, {
    ...
    lineHeight: typeof props.lineHeight === 'number' ? String(props.lineHeight) : props.lineHeight || '1.1',
  });
  container.appendChild(probe);
  const computed = window.getComputedStyle(probe);
  let fontSizePx = parseFloat(computed.fontSize) || 48;
  const fontFamily = computed.fontFamily || 'Newsreader, Georgia, serif';
  const fontWeight = computed.fontWeight || String(props.fontWeight || 500);
  let letterSpacing = computed.letterSpacing === 'normal' ? 0 : parseFloat(computed.letterSpacing) || 0;
  let lineHeight = parseFloat(computed.lineHeight);
  if (!Number.isFinite(lineHeight)) {
    lineHeight = fontSizePx * (typeof props.lineHeight === 'number' ? props.lineHeight : 1.12);
  }
  probe.remove();
  ...
  ctx.fillStyle = props.color || '#183028';
  ...
  const startX = align === 'center' ? width / 2 : align === 'right' ? width - 2 : 2;
  const startY = height / 2 - (lineHeight * (lines.length - 1)) / 2;

  lines.forEach((line, index) => drawLine(ctx, line, startX, startY + index * lineHeight, letterSpacing, align));
