      ...
      <div className="space-y-6 print:p-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-2 text-xs">
            ...
            <div className="text-sm font-bold text-slate-900 dark:text-white">{booking.guestName}</div>
            <div className="text-slate-600 dark:text-slate-300">{booking.guestEmail}</div>
            <div className="text-slate-600 dark:text-slate-300">{booking.guestPhone || '+91 Not provided'}</div>
            {booking.roomNumber && (
              <div className="pt-2 flex items-center gap-2 text-emerald-700 dark:text-emerald-400 font-semibold">
          ...
          <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-2 text-xs">
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              <div>
                ...
                <div className="font-semibold text-slate-900 dark:text-white">{formatDate(booking.checkInDate)}</div>
                <div className="text-slate-400 text-[10px]">From 14:00 hrs</div>
              </div>
              <div>
                <div className="text-slate-500 text-[10px]">CHECK-OUT</div>
                <div className="font-semibold text-slate-900 dark:text-white">{formatDate(booking.checkOutDate)}</div>
        ...
        <div className="p-5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#151D28] space-y-3">
          <div className="font-semibold text-slate-900 dark:text-white text-sm border-b border-slate-100 dark:border-slate-800 pb-2">
          ...
          <div className="space-y-1.5 text-xs">
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-2 flex justify-between font-semibold text-sm">
              <span className="text-slate-900 dark:text-white">Total Booking Value</span>
              <span className="text-emerald-700 dark:text-emerald-400 font-bold">{formatINR(booking.totalAmount)}</span>
            </div>
            ...
            {booking.outstandingBalance > 0 ? (
              <div className="flex justify-between text-xs text-amber-600 dark:text-amber-400 font-bold bg-amber-50 dark:bg-amber-950/40 p-2 rounded-lg">
                <span>Outstanding Balance at Check-in</span>
                <span>{formatINR(booking.outstandingBalance)}</span>
              </div>
            ) : (
              <div className="flex items-center gap-1.5 text-xs text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 p-2 rounded-lg font-medium">
        ...
        <div className="text-xs text-slate-500 dark:text-slate-400 space-y-1 bg-emerald-50/40 dark:bg-emerald-950/20 p-3.5 rounded-xl border border-emerald-100 dark:border-emerald-900/30">
          <div className="font-semibold text-emerald-900 dark:text-emerald-300">Voucher Inclusions:</div>
