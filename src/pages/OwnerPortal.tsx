...
export const OwnerPortal: React.FC<{ onNavigate: (view: string) => void }> = ({ onNavigate }) => {
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="rounded-3xl bg-gradient-to-r from-emerald-900 via-teal-900 to-emerald-950 p-6 sm:p-8 text-white border border-emerald-700/50 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Badge variant="emerald" className="flex items-center gap-1 font-mono text-[11px] bg-white/20 text-emerald-200 border-white/30">
              <Crown className="w-3.5 h-3.5 text-emerald-300" />
              EXECUTIVE CHAIRMAN & OWNER SUITE
            </Badge>
            <span className="text-xs text-emerald-200">
              Principal: <strong className="text-white">{user?.name || 'Siddharth Varma'}</strong>
            </span>
          </div>
          <h1 className="text-2xl sm:text-4xl font-serif font-bold text-white tracking-tight">
            Kaveri Hospitality Command Center
          </h1>
          <p className="text-xs text-emerald-100/90 max-w-2xl leading-relaxed">
            Consolidated enterprise revenue, multi-property asset yield, cross-state portfolio governance, and high-net-worth patron directory.
          </p>
        </div>

        {/* Grand Total Revenue Ticker Banner */}
        <div className="p-4 sm:p-5 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 shadow-xl space-y-1 text-right shrink-0">
          <div className="text-[10px] uppercase font-bold tracking-widest text-emerald-300">
            Chain Grand Total Realized Revenue
          </div>
          <div className="text-3xl sm:text-4xl font-serif font-bold text-white tracking-tight">
            {formatINR(chainGrandTotal)}
          </div>
          <div className="text-[11px] text-emerald-300 flex items-center justify-end gap-1 font-medium">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {propertyRevenues.map((p) => (
          <Card
            key={p.id}
            className="p-6 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm hover:shadow-lg transition-all space-y-4"
          >
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="text-lg font-serif font-bold text-slate-900 dark:text-white mt-1">
                  {p.name}
                </h3>
              </div>
            ...
            <div className="space-y-1">
              <div className="text-xs text-slate-500">Gross Realized Tariff</div>
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800 grid grid-cols-2 gap-2 text-xs text-slate-600 dark:text-slate-400">
              <div>
                <span className="font-semibold text-slate-900 dark:text-white">{p.bookingCount}</span>
                <span className="block text-[10px]">Total Stays</span>
              </div>
              <div>
                <span className="font-semibold text-slate-900 dark:text-white">{p.bookedNights}</span>
                <span className="block text-[10px]">Room Nights Sold</span>
              </div>
            </div>
      ...
      <Card className="rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/60 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full h-10 pl-10 pr-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs focus:ring-1 focus:ring-emerald-500 text-slate-900 dark:text-white"
            />
          </div>
        </div>
                ...
                return (
                  <tr key={g.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="py-4 px-4">
                      <div className="font-bold text-slate-900 dark:text-white text-sm">{g.name}</div>
                      <div className="text-[10px] text-slate-400">ID: {g.id}</div>
                    </td>
                    <td className="py-4 px-4">
                      <div className="text-slate-900 dark:text-white font-medium">{g.email}</div>
                      <div className="text-slate-500 text-[11px]">{g.phone || '+91 98450 12345'}</div>
                    </td>
                    <td className="py-4 px-4 font-bold text-slate-900 dark:text-white">
                      {g.lifetimeNights || 0} Nights
                    </td>
                    <td className="py-4 px-4 font-serif font-bold text-emerald-700 dark:text-emerald-400 text-sm">
                      {formatINR(spend)}
                    </td>
                    <td className="py-4 px-4">
                      {isVIP ? (
