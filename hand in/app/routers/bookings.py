...
# WARNING: This lock only protects within a single process. In production with
# multiple workers (e.g. gunicorn --workers N), each worker gets its own lock.
# The overlap query at booking creation (line ~171) is the real safety net; for
# truly race-free booking, use a database-level advisory lock or exclusion constraint.
    ...
    with booking_lock:
        ...
        if current_user.role.value == "guest":
            ...
            if not target_guest_id:
                guest_rec = db.query(Guest).filter(func.lower(Guest.email) == current_user.email.lower()).first()
                if not guest_rec:
                    guest_rec = Guest(
                    ...
                    db.add(guest_rec)
                    db.flush()
                current_user.guest_id = guest_rec.guest_id
                db.flush()
                target_guest_id = guest_rec.guest_id
        else:
            if req.guest_id:
                ...
                guest_rec = db.query(Guest).filter(func.lower(Guest.email) == current_user.email.lower()).first()
                if not guest_rec:
                    guest_rec = db.query(Guest).first()
                target_guest_id = guest_rec.guest_id if guest_rec else 1
        ...
        if req.deposit:
            ...
            if dep_amount > total_calculated_rate:
                dep_amount = total_calculated_rate
            ...
            payment = Payment(
                ...
                idempotency_key=req.deposit.idempotency_key
            )
            db.add(payment)
