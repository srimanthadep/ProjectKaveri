...
def test_register_guest_six_char_password(client):
    payload = {
        "email": "sixchars@example.com",
        "password": "123456",
        "full_name": "Six Char Test"
    }
    res = client.post("/auth/register", json=payload)
    assert res.status_code == 201
    assert res.json()["email"] == "sixchars@example.com"

def test_register_guest_short_password_422(client):
    payload = {
